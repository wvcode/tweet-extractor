import csv as c
import datetime
import os
import re

import typer


def match_dates(data):
    regex = r"\D*(\d{2}\/\d{2}\/\d{4})\w*"
    matches = re.finditer(regex, data, re.MULTILINE)
    for x, i in enumerate(matches):
        if x == 0:
            start_date = i[1]
        else:
            end_date = i[1]
    return start_date, end_date


def match_equipamento(data):
    regex = r"\D*(EPTC-[0-9]*)\s\-\s(.*)"
    matches = re.finditer(regex, data.replace(',', ''), re.MULTILINE)
    for x, i in enumerate(matches):
        cod_equip = i[1]
        end_equip = i[2]
    return cod_equip, end_equip


def main(arquivo: str):
    dados = []
    fonte = 'dados_transito/'
    with open(os.path.join(fonte, arquivo), 'r', encoding='utf-8') as fr:
        rows = c.reader(fr, delimiter=',')
        i = 1
        for line in rows:
            if 'Período' in line[0]:
                start_date, end_date = match_dates(line[0])
                start_date_arr = start_date.split('/')
                date_start = datetime.date(
                    int(start_date_arr[2]), int(start_date_arr[1]), int(start_date_arr[0]))
            elif 'Endereço' in line[0]:
                end_equip = line[0].replace('Endereço: ', '')
            else:
                if ':' in line[1]:
                    sd = date_start
                    for idx, item in enumerate(line):
                        if idx > 1 and idx < (len(line)-1):
                            rec = {'data': sd,
                                   'equipamento': line[0],
                                   'endereco': end_equip,
                                   'horario': line[1],
                                   'transito': item.replace('.', '').replace(',', '.')
                                   }
                            dados.append(rec)
                            sd = sd + datetime.timedelta(days=1)

        with open(os.path.join('dados_transito_processados/', arquivo), 'w', encoding='utf-8', newline='') as fw:
            cw = c.DictWriter(
                fw, fieldnames=['data', 'equipamento', 'endereco', 'horario', 'volume'])
            cw.writeheader()
            cw.writerows(dados)


# =================================================================================================
# Main Routine
# =================================================================================================
if __name__ == "__main__":
    typer.run(main)
